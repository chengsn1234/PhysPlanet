# 通用轮地力学
####################
## 功能：Bekker-Wong 法向/纵向力、地形采样、轮轴方向计算
## 来源：mars_sim TerramechanicsPlugin.cpp；(cpp:437) 表示原始代码行号
####################
...
# soil_params_map 7 通道顺序，生成端和消费端共用。
...
PARAM = {
    ...
    "sinkage_max": 0.15, "sinkage_step": 0.01,   # .hh:236,240（三点法在粗糙地不稳，暂不放松）
...
def _clamp_symmetric(x: torch.Tensor, limit: torch.Tensor) -> torch.Tensor:
...
def compute_terramechanics(
    ...
    # 返回 force_z(世界+Z,含阻尼,非负) / force_forward(纵向标量,调用方沿轮前向旋世界) / Fn(原始法向) / slip / sinkage / state
    ...
    passive_drag = _clamp_symmetric(-PARAM["LongitudinalDampCoef"] * v_forward, passive_limit)
    ...
    return {
        ...
        "force_forward": Fdp,
        "Fn": Fn,
        "Mr": Mr,                 # 滚动阻力矩（轮轴系 y 轴），试验台对比/轮矩传感用
...
def solve_static_sinkage(fn_target, r: float, b: float, h: float = 0.0, soil: dict | None = None):
    ...
    # fn_target 标量 → 返回 float（向后兼容）；(E,N) tensor + soil → 返回 (E,N) per-wheel sinkage_eq。
...
def clamp_symmetric(x, limit):
    return torch.maximum(torch.minimum(x, limit), -limit)


def bilinear_sample(grid, xy, terrain_size):
    H, W = grid.shape[:2]
    mpp = terrain_size / max(H - 1, 1)
    c = xy[:, 0] / mpp
    r = xy[:, 1] / mpp
...
def bilinear_ground_z(xy, heights, terrain_size):
    return bilinear_sample(heights, xy, terrain_size)


def root_axes_w(root_quat_w, device):
...
def steered_wheel_axes_w(root_quat_w, steer, device):
...
def setup_rover_terramechanics(
    robot,
    wheel_body_names,
    wheel_joint_ids,
    wheel_r,
    ...
    body_id_map = dict(zip(robot.body_names, range(robot.num_bodies)))
    wheel_body_ids = [body_id_map[name] for name in wheel_body_names]
    num_wheels = len(wheel_body_ids)
    total_mass = float(robot.data.default_mass.sum()) / num_envs
    fn_ave = total_mass * float(gravity) / num_wheels
    sinkage_eq = solve_static_sinkage(fn_ave, wheel_r, wheel_b, h=wheel_lug_h)
    tm_state = {"sinkage_prev": torch.full((num_envs, num_wheels), sinkage_eq, device=robot.device)}
    return {
...
def build_terramechanics_forces(robot, tm, terrain, wheel_forward_w, wheel_right_w):
    num_envs = terrain["num_envs"]
    num_wheels = tm["num_wheels"]
    wheel_body_ids = tm["wheel_body_ids"]
    wheel_joint_ids = tm["wheel_joint_ids"]
    wheel_r = tm["wheel_r"]
    wheel_b = tm["wheel_b"]
    fn_ave = tm["fn_ave"]
    ...
    omega = robot.data.joint_vel[:, wheel_joint_ids]
    # Scene uses one global /World/terrain; map coordinates are world XY.
    wheel_xy = wheel_pos_w[..., :2]
    wheel_xy_flat = wheel_xy.reshape(-1, 2)

    ground_z_t = bilinear_ground_z(wheel_xy_flat, heights_t, terrain_size).reshape(num_envs, num_wheels)
    sinkage = torch.clamp(wheel_r - (wheel_pos_w[..., 2] - ground_z_t), min=0.0)

    soil = None
    if soil_params_t is not None:
        wheel_soil = bilinear_sample(soil_params_t, wheel_xy_flat, terrain_size).reshape(num_envs, num_wheels, len(SOIL_KEYS))
        soil = dict(zip(SOIL_KEYS, wheel_soil.unbind(-1)))

    wheel_vel_w = robot.data.body_lin_vel_w[:, wheel_body_ids, :]
    v_fwd = (wheel_vel_w * wheel_forward_w).sum(dim=-1)
    v_side = (wheel_vel_w * wheel_right_w).sum(dim=-1)
    vz = wheel_vel_w[..., 2]
    ...
    tm_out = compute_terramechanics(
        ...
        v_fwd,
        vz,
        wheel={"r": wheel_r, "b": wheel_b, "h": tm.get("wheel_lug_h", 0.0)},
        fn_ave=fn_ave,
        state=tm["tm_state"],
        soil=soil,
    )
    tm["tm_state"] = tm_out["state"]

    forces = wheel_forward_w * tm_out["force_forward"][..., None]
    side_drag = clamp_symmetric(-PARAM["LateralDampCoef"] * v_side, tm_out["Fn"] * tm_out["tan_phi"])
    forces += wheel_right_w * side_drag[..., None]
    forces[:, :, 2] += tm_out["force_z"]

    dbg = {
        "wheel_pos_w": wheel_pos_w,
        "ground_z_t": ground_z_t,
        "v_fwd": v_fwd,
        "omega": omega,
        "vz": vz,
        "side_drag": side_drag,
