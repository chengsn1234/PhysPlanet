...
class ElevationMap:
    ...
    def __init__(self, param: Parameter):
        ...
        # Resolve weight_file path
        weight_file = param.weight_file
        if weight_file and not os.path.isabs(weight_file):
            from ament_index_python.packages import get_package_share_directory
            pkg_root = get_package_share_directory("elevation_mapping_cupy")
            weight_file = os.path.join(pkg_root, weight_file)
        ...
        if plugin_config_file and not os.path.isabs(plugin_config_file):
            from ament_index_python.packages import get_package_share_directory
            pkg_root = get_package_share_directory("elevation_mapping_cupy")
            plugin_config_file = os.path.join(pkg_root, plugin_config_file)
    ...
    def pad_value(self, x, shift_value, idx=None, value=0.0):
        ...
        Args:
            ...
            shift_value (cupy._core.core.ndarray): [delta_x, delta_y] in world coordinates
    ...
    def shift_map_xy(self, delta_pixel):
        ...
        if cp.abs(shift_value).sum() == 0:
            return
        with self.map_lock:
            self.elevation_map = cp.roll(self.elevation_map, shift_value, axis=(2, 1))
    ...
    def compile_image_kernels(self):
        ...
        print(f"subscriber_cfg: {self.param.subscriber_cfg}")
        for config in self.param.subscriber_cfg.values():
            print(f"config: {config}")
            if "data_type" in config and config["data_type"] == "image":
    ...
    def input_pointcloud(
        ...
        # Transform points to world frame and check z
        R_np = cp.asarray(R, dtype=self.data_type).flatten()
        t_np = cp.asarray(t, dtype=self.data_type)
        ...
        with self.map_lock:
            if name == "elevation":
                ...
                self.plugin_manager.update_with_name(
                    ...
                    self.base_rotation,
                    {**self.semantic_map.elements_to_shift, "center": self.center},
                )
                ...
                return
        # Need 180 degree rotation + transpose to match grid_map convention (row→X, col→Y)
        m = xp.flip(m, 0)
        m = xp.flip(m, 1)
        m = m.T
        if use_stream:
