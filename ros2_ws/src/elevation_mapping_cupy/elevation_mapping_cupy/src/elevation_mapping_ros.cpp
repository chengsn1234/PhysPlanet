    ...
    for (const auto& sub_name : unique_sub_names) {  
      ...
      if(this->get_parameter("subscribers." + sub_name + ".data_type", data_type)){
          ...
          if(data_type == "image"){
            ...
            rmw_qos_profile_t pc_qos_profile = rmw_qos_profile_sensor_data;
            auto pc_qos = rclcpp::QoS(rclcpp::QoSInitialization(pc_qos_profile.history, pc_qos_profile.depth), pc_qos_profile);
            auto sub = this->create_subscription<sensor_msgs::msg::PointCloud2>(pointcloud_topic, pc_qos, callback_transport);
...
void ElevationMappingNode::inputPointCloud(const sensor_msgs::msg::PointCloud2::ConstSharedPtr cloud,
    ...
    try {
        ...
        RCLCPP_ERROR(this->get_logger(), "TF FAIL: %s -> %s: %s", sensorFrameId.c_str(), mapFrameId_.c_str(), ex.what());
        return;
    }
