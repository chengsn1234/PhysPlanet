...
class ElevationMappingNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("=== Got package directory ===")
        ...
        self.get_logger().info("=== About to create Parameter ===")
        ...
        self.get_logger().info("=== Created Parameter ===")

        # Read ROS parameters (including YAML)
        self.initialize_ros()
        self.get_logger().info(f"my_subscribers after initialize_ros: {self.my_subscribers}")
        self.set_param_values_from_ros()

        # Overwrite subscriber_cfg from loaded YAML
        self.param.subscriber_cfg = self.my_subscribers
        self.get_logger().info(f"param.subscriber_cfg: {self.param.subscriber_cfg}")
    ...
    def initialize_ros(self) -> None:
        self.get_logger().info("=== Starting initialize_ros ===")
        self._tf_buffer = tf2_ros.Buffer()
        self.get_logger().info("=== Created tf_buffer ===")
        self._listener = tf2_ros.TransformListener(self._tf_buffer, self)
        self.get_logger().info("=== Created tf_listener ===")
        self.get_ros_params()

    def get_ros_params(self) -> None:
        # 打印所有参数名
        all_params = self.get_parameter_names()
        print(f"[DEBUG] All parameter names: {all_params}", flush=True)
        ...
        self.get_logger().info("=== Before get_parameters_by_prefix ===")
        try:
            subscribers_params = self.get_parameters_by_prefix('subscribers')
            self.get_logger().info(f"subscribers_params: {subscribers_params}")
        except Exception as e:
            self.get_logger().error(f"Exception in get_parameters_by_prefix: {e}")
            subscribers_params = {}
        self.get_logger().info("=== After get_parameters_by_prefix ===")
        self.my_subscribers = {}
        ...
        self.get_logger().info(f"my_subscribers: {self.my_subscribers}")
