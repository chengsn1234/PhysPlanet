#!/usr/bin/env python3
"""Convert zhurong depth image to PointCloud2 for elevation_mapping."""
import numpy as np
import rclpy
...
class DepthToPointCloud(Node):
    def __init__(self):
        super().__init__('zhurong_depth_to_pointcloud')
        ...
        info_sub = self.create_subscription(
            CameraInfo, f'/{prefix}/zhurong_camera_nav_info',
            self._info_cb, 10)

        self.pc_pub = self.create_publisher(
            PointCloud2, f'/{prefix}/zhurong_camera_nav_points', 10)

        depth_qos = QoSProfile(
            reliability=ReliabilityPolicy.BEST_EFFORT,
            history=HistoryPolicy.KEEP_LAST, depth=1)
        self.create_subscription(
            Image, f'/{prefix}/zhurong_camera_nav_depth',
